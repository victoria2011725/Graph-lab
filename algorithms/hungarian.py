import numpy as np

def hungarian(C_orig):
    C = C_orig.copy()
    n = C.shape[0]
    
    C -= C.min(axis=1, keepdims=True)
    C -= C.min(axis=0, keepdims=True)
    
    marked = np.zeros((n, n), dtype=int)
    row_covered = np.zeros(n, dtype=bool)
    col_covered = np.zeros(n, dtype=bool)

    for r in range(n):
        for c in range(n):
            if C[r, c] == 0 and not row_covered[r] and not col_covered[c]:
                marked[r, c] = 1
                row_covered[r] = True
                col_covered[c] = True
    
    row_covered.fill(False)
    col_covered.fill(False)
    
    step = 3
    path_start = None

    while step != 7:
        if step == 3: 
            col_covered.fill(False)
            for r in range(n):
                for c in range(n):
                    if marked[r, c] == 1:
                        col_covered[c] = True
            step = 7 if col_covered.sum() >= n else 4

        elif step == 4: 
            r_idx, c_idx = -1, -1
            for r in range(n):
                if not row_covered[r]:
                    for c in range(n):
                        if not col_covered[c] and C[r, c] == 0:
                            r_idx, c_idx = r, c
                            break
                    if r_idx != -1: break
            
            if r_idx == -1:
                step = 5 
            else:
                marked[r_idx, c_idx] = 2 
                star_col = -1
                for c in range(n):
                    if marked[r_idx, c] == 1:
                        star_col = c
                        break
                
                if star_col != -1:
                    row_covered[r_idx] = True
                    col_covered[star_col] = False
                    step = 4
                else:
                    path_start = (r_idx, c_idx)
                    step = 6

        elif step == 5:
            uncovered_mask = np.outer(~row_covered, ~col_covered)
            if not np.any(uncovered_mask):
                step = 7
                continue
            min_val = C[uncovered_mask].min()
            C[row_covered, :] += min_val
            C[:, ~col_covered] -= min_val
            step = 4

        elif step == 6:
            path = [path_start]
            while True:
                last_r, last_c = path[-1]
                star_r = -1
                for r in range(n):
                    if marked[r, last_c] == 1:
                        star_r = r
                        break
                if star_r == -1: break
                path.append((star_r, last_c))
                
                last_r, last_c = path[-1]
                prime_c = -1
                for c in range(n):
                    if marked[last_r, c] == 2:
                        prime_c = c
                        break
                path.append((last_r, prime_c))

            for r, c in path:
                marked[r, c] = 1 if marked[r, c] == 2 else 0
            
            marked[marked == 2] = 0
            row_covered.fill(False)
            col_covered.fill(False)
            step = 3

    return marked == 1